...
from config.config import get_settings, require, RUNS_DIR, BUNDLE_DIR, configure_root_logging
...
s = get_settings()
logger = configure_root_logging(s, name="search_flow_news")
...
def build_query_non_dz() -> str:
    """
    Q2 = broad context query with Slovakia terms.
    """
...
def serpapi_google_news_search(query: str, api_key: str, num: int, hl: str, gl: str, when: str) -> dict:
    ...
    while len(all_results) < num:
        ...
        if "error" in data:
            logger.warning(f"SerpAPI error for query={query!r}: {data['error']}")
            break
...
def main() -> None:
    ...
    if not api_key:
        raise SystemExit("Missing SERPAPI_KEY in .env / environment.")

    # Tuning
    # --- toto sú tvoje pôvodné hardcoded hodnoty, len ich nahradíme args ---
    hl = args.hl
    gl = args.gl
    num = args.num
    ...
    try:
        ...
        save_json(q1_raw, run_dir / f"news_q1_raw.json")
    except Exception:
    ...
    # Q2: broad Algeria-context query
    q2 = build_query_non_dz()
    logger.info(f"[2/2] Q2 (NON-DZ) : {q2}")
    try:
        ...
        save_json(q2_raw, run_dir / f"news_q2_raw.json")
    except Exception:
        logger.exception("SerpAPI call failed for Q2")
        raise

    # Clean (limit to num after cleaning)
    ...
    save_json(q1_clean, run_dir / f"news_q1_clean.json")
    save_json(q2_clean, run_dir / f"news_q2_clean.json")
    save_json({"news_results": combined}, run_dir / "news_combined.json")

    # Stats
    logger.info("\n--- STATS (RAW) ---")
    logger.info(f"Q1 raw news_results: {len(extract_news_items(q1_raw))} (need >= {num} for 'full')")
    logger.info(f"Q2 raw news_results: {len(extract_news_items(q2_raw))} (need >= {num} for 'full')")
    logger.info("\n--- STATS (CLEAN) ---")
    logger.info(f"Q1 clean news_results: {len(extract_news_items(q1_clean))}, preferred={count_preferred_in_results(q1_clean)}, domains={unique_domain_count(q1_clean)}")
    logger.info(f"Q2 clean news_results: {len(extract_news_items(q2_clean))}, preferred={count_preferred_in_results(q2_clean)}, domains={unique_domain_count(q2_clean)}")

    #any_pref = contains_preferred_links(extract_links_from_news(q1_clean) + extract_links_from_news(q2_clean))
    any_pref = contains_preferred_links(
        extract_links_from_news({"news_results": combined})
    )

    logger.info(f"Preferred present in Q1+Q2 (clean): {any_pref}")
    logger.info("\n--- STATS (Deduplicated) ---")
    logger.info(f"Unique articles after dedup: {len(combined)}")
    logger.info("\n--- DOMAIN STATS ---")
    ...
    for item in combined[:5]:
        logger.info(
            f"{domain_of(item.get('link'))} | {item.get('title')}"
        )
    ...
    logger.info(f"Fallback check: {fallback_reason(q1_raw, q2_raw, combined, num)}")

    if should_run_preferred_fallback(q1_raw, q2_raw, combined, num=num):
        time.sleep(1.0)
        q3 = build_query_preferred_fallback()
        ...
        try:
            ...
            save_json(q3_raw, run_dir / f"news_q3_raw.json")
        except Exception:
            logger.exception("SerpAPI call failed for Q3")
            raise

        q3_clean = clean_news_results(q3_raw, limit=num)
        save_json(q3_clean, run_dir / f"news_q3_clean.json")

        logger.info("\n--- STATS (FALLBACK CLEAN) ---")
        logger.info(f"Q3 clean news_results: {len(extract_news_items(q3_clean))}, preferred={count_preferred_in_results(q3_clean)}, domains={unique_domain_count(q3_clean)}")
    else:
    ...
    # save_json(bundle, str(run_dir / f"news_bundle.json"))
    save_json(bundle, run_dir / "news_bundle.json")
    update_latest_symlink(run_dir)

    logger.info(f"\nSaved bundle: news_bundle_{ts}.json")
    logger.info("Done.")
